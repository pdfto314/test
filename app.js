...
async function listGithubDir(path){
  const api = `https://api.github.com/repos/${OWNER}/${REPO}/contents/${encodePath(path)}?ref=${BRANCH}`;
...
function promptSaveScene(){
  ...
  const name = prompt("Nome do set (ex: Floresta + Chuva):");
  if (!name) return;
  SCENES[name] = snap;
  saveScenes();
  renderSceneSelect();
